...
async function fetchRecentMessages(channel, commandMessageId) {
    const messages = [];
    let before;
    ...
    while (messages.length < MAX_MESSAGES_TO_SCAN) {
        ...
        const recentMessages = fetched.filter(msg => {
            ...
            return !msg.pinned && msg.id !== commandMessageId && canBulkDelete;
        });
...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (!Number.isInteger(porcentaje) || porcentaje < 1 || porcentaje > 100) {
            return message.reply({ embeds: [new EmbedBuilder().setColor('Red').setDescription('⚠️ **Este comando está incompleto o es inválido.**\n\n**Estructura:**\n`dex clear número`\n*(El número debe ser un porcentaje entre 1 y 100. Ejemplo: `dex clear 100` limpia todo lo reciente posible y `dex clear 50` limpia la mitad.)*')] });
        }

        const recentMessages = await fetchRecentMessages(message.channel, message.id);
        const cantidad = Math.ceil(recentMessages.length * (porcentaje / 100));
